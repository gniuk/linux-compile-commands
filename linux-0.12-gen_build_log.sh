...
for _dir in ${_subdirs[@]}; do
  LANG=en_US.UTF-8 LANGUAGE=en make -C ${_dir} V=1 -j1 --dry-run |& tee -a build-log.txt
done
LANG=en_US.UTF-8 LANGUAGE=en make V=1 -j1 --dry-run init/main.o |& tee -a build-log.txt
